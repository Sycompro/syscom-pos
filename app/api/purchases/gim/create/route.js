...
export async function POST(req) {
    try {
        ...
        logger.info(`[API/Purchases/Gim/Create] Recibida solicitud de GIM para ${session.user.company}`, { body });
        ...
        const result = await NavaPurchaseService.createGIM(body, session.user.company, session.user.id || 'ADMIN');

        return NextResponse.json(result);

    } catch (err) {
        logger.error(`[API/Purchases/Gim/Create] Error crítico al registrar GIM: ${err.message}`, { stack: err.stack });
        return NextResponse.json({ 
            error: 'Error interno al registrar la Nota de Ingreso',
            details: err.message 
        }, { status: 500 });
    }
}
